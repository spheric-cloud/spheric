...
// ExtractDiskType extracts the applied configuration owned by fieldManager from
// diskType. If no managedFields are found in diskType for fieldManager, a
// DiskTypeApplyConfiguration is returned with only the Name, Namespace (if applicable),
// APIVersion and Kind populated. It is possible that no managed fields were found for because other
// field managers have taken ownership of all the fields previously owned by fieldManager, or because
// the fieldManager never owned fields any fields.
// diskType must be a unmodified DiskType API object that was retrieved from the Kubernetes API.
// ExtractDiskType provides a way to perform a extract/modify-in-place/apply workflow.
...
func ExtractDiskType(diskType *v1alpha1.DiskType, fieldManager string) (*DiskTypeApplyConfiguration, error) {
	return extractDiskType(diskType, fieldManager, "")
}

// ExtractDiskTypeStatus is the same as ExtractDiskType except
// that it extracts the status subresource applied configuration.
// Experimental!
func ExtractDiskTypeStatus(diskType *v1alpha1.DiskType, fieldManager string) (*DiskTypeApplyConfiguration, error) {
	return extractDiskType(diskType, fieldManager, "status")
}

func extractDiskType(diskType *v1alpha1.DiskType, fieldManager string, subresource string) (*DiskTypeApplyConfiguration, error) {
	b := &DiskTypeApplyConfiguration{}
	err := managedfields.ExtractInto(diskType, internal.Parser().Type("cloud.spheric.spheric.api.core.v1alpha1.DiskType"), fieldManager, b, subresource)
	if err != nil {
		return nil, err
	}
	b.WithName(diskType.Name)
...
func (b *DiskTypeApplyConfiguration) ensureObjectMetaApplyConfigurationExists() {
