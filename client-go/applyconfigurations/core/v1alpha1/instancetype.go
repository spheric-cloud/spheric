...
// ExtractInstanceType extracts the applied configuration owned by fieldManager from
// instanceType. If no managedFields are found in instanceType for fieldManager, a
// InstanceTypeApplyConfiguration is returned with only the Name, Namespace (if applicable),
// APIVersion and Kind populated. It is possible that no managed fields were found for because other
// field managers have taken ownership of all the fields previously owned by fieldManager, or because
// the fieldManager never owned fields any fields.
// instanceType must be a unmodified InstanceType API object that was retrieved from the Kubernetes API.
// ExtractInstanceType provides a way to perform a extract/modify-in-place/apply workflow.
...
func ExtractInstanceType(instanceType *v1alpha1.InstanceType, fieldManager string) (*InstanceTypeApplyConfiguration, error) {
	return extractInstanceType(instanceType, fieldManager, "")
}

// ExtractInstanceTypeStatus is the same as ExtractInstanceType except
...
func ExtractInstanceTypeStatus(instanceType *v1alpha1.InstanceType, fieldManager string) (*InstanceTypeApplyConfiguration, error) {
	return extractInstanceType(instanceType, fieldManager, "status")
}

func extractInstanceType(instanceType *v1alpha1.InstanceType, fieldManager string, subresource string) (*InstanceTypeApplyConfiguration, error) {
	b := &InstanceTypeApplyConfiguration{}
	err := managedfields.ExtractInto(instanceType, internal.Parser().Type("cloud.spheric.spheric.api.core.v1alpha1.InstanceType"), fieldManager, b, subresource)
	if err != nil {
		return nil, err
	}
	b.WithName(instanceType.Name)
