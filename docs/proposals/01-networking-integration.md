...
structure of `spheric`.

## Motivation

Without networking, any machine / process running inside a datacenter cannot interact / affect the outside world.
Networking is a crucial component that has to be implemented for spheric to have business value. In a full-fledged
state, networking also enables security to the outside world and within a datacenter itself.

The basic use case we want to implement with spheric is a machine that can access the internet and can be reached from
the internet.
...
As spheric is Kubernetes-API, it should integrate nicely within the existing ecosystem. Some API design choices are made
...
[//]: # (@formatter:off)
```yaml
apiVersion: ipam.spheric.cloud/v1alpha1
kind: Prefix
metadata:
  namespace: default
  name: my-root-prefix
...
[//]: # (@formatter:off)
```yaml
apiVersion: networking.spheric.cloud
kind: VirtualIP
metadata:
  namespace: default
  name: my-virtual-ip
...
### Kubernetes (Gardener) integration on top of spheric
