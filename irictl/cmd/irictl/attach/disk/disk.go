...
func Run(ctx context.Context, streams clicommon.Streams, client iri.RuntimeServiceClient, opts Options) error {
	...
	if _, err := client.AttachDisk(ctx, &iri.AttachDiskRequest{Disk: volume, InstanceId: opts.InstanceID}); err != nil {
		return err
	}

	_, _ = fmt.Fprintf(streams.Out, "Attached disk %s to instance %s\n", volume.Name, opts.InstanceID)
	return nil
}
