...
func Run(ctx context.Context, streams clicommon.Streams, client sri.VolumeRuntimeClient, ids []string) error {
	for _, id := range ids {
		if _, err := client.DeleteVolume(ctx, &sri.DeleteVolumeRequest{
			VolumeId: id,
		}); err != nil {
