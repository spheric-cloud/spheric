...
var log = logf.Log.WithName("spherelet").WithName("server")

type Options struct {
	// RuntimeService is the instance runtime service.
	RuntimeService instance.RuntimeService
	...
	// {TempDir}/spheric-fleet-server/serving-certs. The key and certificate
...
func setOptionsDefaults(opts *Options) {
	...
	if opts.CertDir == "" {
		opts.CertDir = filepath.Join(os.TempDir(), "spherelet-server", "serving-certs")
	}
...
type FleetRequestAttr struct {
	FleetName string
}

func (m FleetRequestAttr) GetRequestAttributes(u user.Info, req *http.Request) authorizer.Attributes {
	return authorizer.AttributesRecord{
		User:            u,
		Verb:            getAPIVerb(req.Method),
		Namespace:       "",
		APIGroup:        corev1alpha1.SchemeGroupVersion.Group,
...
func NewAuth(cfg *rest.Config, opts AuthOptions) (Auth, error) {
	...
	return authWrapper{
		Request:                 authN,
		RequestAttributesGetter: FleetRequestAttr{opts.FleetName},
		Authorizer:              authZ,
	}, nil
}
...
func New(cfg *rest.Config, opts Options) (*Server, error) {
	...
	if opts.RuntimeService == nil {
		return nil, fmt.Errorf("must specify RuntimeService")
	}
