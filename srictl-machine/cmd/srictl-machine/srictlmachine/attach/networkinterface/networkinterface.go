...
func Run(ctx context.Context, streams clicommon.Streams, client sri.MachineRuntimeClient, opts Options) error {
	data, err := clicommon.ReadFileOrReader(opts.Filename, os.Stdin)
	if err != nil {
		return err
	}

	networkInterface := &sri.NetworkInterface{}
	if err := decoder.Decode(data, networkInterface); err != nil {
		return err
	}

	if _, err := client.AttachNetworkInterface(ctx, &sri.AttachNetworkInterfaceRequest{NetworkInterface: networkInterface}); err != nil {
		return err
	}
